defmodule PhoenixKitWarehouse.Web.InternalOrderIndexLive do
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      ...
      |> assign(:orders, [])
      |> ColumnManagement.assign_column_state(InternalOrderColumnConfig)
  ...
  @impl true
  def handle_url_state(_state, socket), do: assign_orders(socket)
  ...
  defp sortable_column?(column_id) do
    match?(
      %{sortable?: true},
      Map.get(InternalOrderColumnConfig.column_metadata_map(), column_id)
    )
  end
